...
import { Popover, Typography, RadioGroup, Radio, FormControlLabel, Checkbox, FormGroup, Grow } from '@mui/material';
...
const ListingFilter = () => {
  ...
  const handleSelect = (filterType, option) => {
    const existingFilter = selectedFilters.find((filter) => filter.filterType === filterType && filter.option === option);

    if (existingFilter) {
      setSelectedFilters(selectedFilters.filter((filter) => filter !== existingFilter));
    } else {
      if (filters[filterType].type === 'radio') {
        setSelectedFilters([
          ...selectedFilters.filter((filter) => filter.filterType !== filterType),
          { filterType, option },
        ]);
      } else {
  ...
  const handleRemoveFilter = (option) => {
    setSelectedFilters(selectedFilters.filter((filter) => filter.option !== option));
  };
  ...
  const renderFilterOptions = (filter) => {
    ...
    if (type === 'radio') {
        ...
        <RadioGroup>
          {options.map((option, index) => (
            <FormControlLabel
              key={index}
              value={option}
              control={<CustomRadio checked={selectedFilters.some((selected) => selected.option === option)} />}
              label={option}
        ...
        <FormGroup>
          {options.map((option, index) => (
            <FormControlLabel
              ...
              control={
                <CustomCheckbox
                  checked={selectedFilters.some((selected) => selected.option === option)}
      ...
      return (
        <input type="search" placeholder="Search location..." className="filter-location-search" />
      );
    }
  };
  ...
  return (
    <>
      <div className="filter-row">
        ...
        <div className="filter-drop" onClick={(e) => handleClick(e, 'recommended')}>
          <h6>Sort by : Recommended <KeyboardArrowDownIcon /></h6>
        </div>
        ...
        <div className="filter-drop" onClick={(e) => handleClick(e, 'location')}>
          <h6>Location <KeyboardArrowDownIcon /></h6>
        </div>
      ...
      <div className="searched-filter">
        {selectedFilters.map((filter, index) => (
          <h5 key={index}>
            {filter.option} <ClearIcon onClick={() => handleRemoveFilter(filter.option)} />
          </h5>
        ))}
      </div>
    </>
